...
class RawRecordingsDao extends DatabaseAccessor<AppDatabase>
  ...
  Stream<List<RawRecordingsTableData>> watchAll() =>
      (select(rawRecordingsTable)..orderBy([
  ...
  Future<int> deleteGroup(String captureGroupId) => (delete(
    rawRecordingsTable,
  )..where((r) => r.captureGroupId.equals(captureGroupId))).go();
}
